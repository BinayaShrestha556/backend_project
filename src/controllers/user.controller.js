...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const user = await User.create({//creating the user if everything goes correct
    fullname,
    avatar: avatar.url,
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const options = {//this secures the cookies so that it can only be changed using server and not by user or frontend
    httpOnly: true,
    secure: true,
  };
  return res
...
const refreshAccessToken=asyncHandler(async(req,res)=>{
  try {
    ...
    const decodedToken=jwt.verify(incomngRefreshToken,process.env.ACCESS_TOKEN_SECRET)
