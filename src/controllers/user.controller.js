...
const registerUser = asyncHandler(async (req, res) => {
  ...
  try {
      ...
      const avatarLocalPath = req.files.avatar?req.files.avatar[0]:null; //getting the avatar if it exists, the req.files is added by the middleware multer
      if (!avatarLocalPath) return res.status(400).json( {message: "avatar required"});
      const avatar = await uploadOnCloudinary(avatarLocalPath.buffer,"avatars");
      ...
      if (
        ...
        coverImage=await uploadOnCloudinary(coverImageLocal.buffer,"cover_images")
      }
    ...
    console.log(error)
    return res.status(500).json(new ApiError(500,"error somewhere"))
  }
